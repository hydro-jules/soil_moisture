...
kge_smap = np.empty([0])
kge_smos = np.empty([0])
kge_chess = np.empty([0])
kge_s1 = np.empty([0])
kge_tc_merge = np.empty([0])
kge_smap_l4 = np.empty([0])

notnan_sm = np.empty([0])


def calculateKGE(all_sites_RS_SM,all_sites_COSMOS_SM):
    # metrics KGE used can be seen in this paper: http://www.sciencedirect.com/science/article/pii/S0022169412000431
    # Calculate correlation between 2 time series
    correlation = np.ma.corrcoef(np.ma.masked_invalid(all_sites_RS_SM),np.ma.masked_invalid(all_sites_COSMOS_SM))[0,1]
    ##print correlation
    # Calculate bias ratio
    bias = np.mean(all_sites_RS_SM)/np.mean(all_sites_COSMOS_SM)
    ##print bias
    # calculate variability ratio (VR) and coefficient of variation (CV)
    CVgridPET = np.std(all_sites_RS_SM)/np.mean(all_sites_RS_SM)
    CVcatchPET = np.std(all_sites_COSMOS_SM)/np.mean(all_sites_COSMOS_SM)
    VR = CVgridPET/CVcatchPET
    ##print VR
    # calculate modified KGE statistic
    KGE = 1 - math.sqrt((correlation-1)**2 + (bias-1)**2 + (VR-1)**2)
    #print KGE
    return KGE
...
for site in station_names:
     ...
     kge_smap = np.append(kge_smap,calculateKGE(new['smap9km'+site][mask],new[site][mask]))
     kge_smos = np.append(kge_smos,calculateKGE(new['smos1km'+site][mask],new[site][mask]))
     kge_chess = np.append(kge_chess,calculateKGE(new['chess1km'+site][mask],new[site][mask]))
     kge_s1 = np.append(kge_s1,calculateKGE(new['s1'+site][mask],new[site][mask]))
     kge_tc_merge = np.append(kge_tc_merge,calculateKGE(new['tc_merge'+site][mask],new[site][mask]))
     kge_smap_l4 = np.append(kge_smap_l4,calculateKGE(new['smap_l4'+site][mask],new[site][mask]))
...
######plot box plot for correlation#################
######plot box plot for correlation#################
R_box = [kge_smos[mask_nan], kge_smap[mask_nan],kge_smap_l4[mask_nan],kge_chess[mask_nan],kge_tc_merge[mask_nan]]
...
ax1.set_ylabel(r'$KGE$',fontsize = 11)
ax1.grid(True)
fig.autofmt_xdate()
plt.savefig(plot_dir + 'box_KGE_merge_with_seasonality_'+version+'.png',dpi=400,bbox_inches='tight')

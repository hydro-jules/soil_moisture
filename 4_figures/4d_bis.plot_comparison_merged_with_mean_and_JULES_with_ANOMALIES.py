...
if GLDAS == False:
    if hamburg==True:
        ...
        plot_dir = '/prj/hydrojules/users/malngu/plots/plots_for_review/number_4_merged_and_mean_and_jules_with_ANOMALIES/'+method + '_' + bulk_den + '/'
else:
...
#     # anomaly = ts.calc_anomaly(new[s], window_size=30)
    anomaly = new[s] - mv.moving_average(new[s], window_size=window_size,min_obs=6)
    new[s] = anomaly
...
mask_nan = ~np.isnan(rvalue_smap)&~np.isnan(rvalue_smos)&~np.isnan(rvalue_tc_merge)&~np.isnan(rvalue_s1)&~np.isnan(rvalue_chess) &~np.isnan(rvalue_mean)
...
######plot box plot for correlation#################
######plot box plot for correlation#################
R_box = [rvalue_chess[mask_nan],rvalue_tc_merge[mask_nan],rvalue_s1[mask_nan],rvalue_mean[mask_nan]]
...
ax1.set_xticklabels(['JULES-CHESS','Merge','Average1','Average2'],fontsize = 11)
ax1.set_ylim([0,1.1])
ax1.yaxis.set_tick_params(labelsize=11)
ax1.set_ylabel(r'$R$',fontsize = 11)
ax1.grid(True)
fig.autofmt_xdate()
plt.savefig(plot_dir + 'box_corrleation_merge_and_mean_and_jules_no_seasonality_'+version+'.png',dpi=400,bbox_inches='tight')
# plt.savefig('box_corrleation_merge_no_seasonality.png',dpi=400,bbox_inches='tight')
plt.close()


######plot box plot for ubRMSE#################
R_box = [crmse_chess[mask_nan],crmse_tc_merge[mask_nan],crmse_s1[mask_nan],crmse_mean[mask_nan]]
...
ax1.grid(True)
fig.autofmt_xdate()
plt.savefig(plot_dir + 'box_crmse_merge_and_mean_and_jules_no_seasonality_'+version+'.png',dpi=400,bbox_inches='tight')
...
R_box = [rmse_chess[mask_nan],rmse_tc_merge[mask_nan],rmse_s1[mask_nan],rmse_mean[mask_nan]]
...
ax1.grid(True)
fig.autofmt_xdate()
plt.savefig(plot_dir +'box_rmse_merge_and_mean_and_jules_no_seasonality_'+version+'.png',dpi=400,bbox_inches='tight')
# plt.savefig('box_rmse_merge_no_seasonality.png',dpi=400,bbox_inches='tight')
plt.close()


R_box = [bias_chess[mask_nan],bias_tc_merge[mask_nan],bias_s1[mask_nan],bias_mean[mask_nan]]
...
ax1.grid(True)
fig.autofmt_xdate()
plt.savefig(plot_dir +'box_bias_merge_and_mean_and_jules_no_seasonality_'+version+'.png',dpi=400,bbox_inches='tight')

...
for k in range(41):
   ...
   smap9km['smap9km' + station_names[k]+'_ANOM'] = smap9km['smap9km' + station_names[k]]-np.mean(smap9km['smap9km' + station_names[k]])
   listColsmap9km.append('smap9km' + station_names[k]+'_ANOM')
smap9km['smap9km_avg'] = smap9km[listColsmap9km].mean(axis=1)

smos1km = extract_point(file_smos1km,date_range_cf,41,lat,lon,'sm',1)
listColsmos1km = []
for k in range(41):
   smos1km = smos1km.rename(columns={k:'smos1km' + station_names[k] })
   smos1km['smos1km' + station_names[k]+'_ANOM'] = smos1km['smos1km' + station_names[k]]-np.mean(smos1km['smos1km' + station_names[k]])
   listColsmos1km.append('smos1km' + station_names[k]+'_ANOM')
smos1km['smos1km_avg'] = smos1km[listColsmos1km].mean(axis=1)
...
for k in range(41):
   ...
   chess1km['chess1km' + station_names[k]+'_ANOM'] = chess1km['chess1km' + station_names[k]]-np.mean(chess1km['chess1km' + station_names[k]])
   listColchess.append('chess1km' + station_names[k]+'_ANOM')
chess1km['chess1km_avg'] = chess1km[listColchess].mean(axis=1)
...
for k in range(41):
   ...
   tc_merge['tc_merge' + station_names[k]+'_ANOM'] = tc_merge['tc_merge' + station_names[k]]-np.mean(tc_merge['tc_merge' + station_names[k]])
   listColtc_merge.append('tc_merge' + station_names[k]+'_ANOM')
tc_merge['tc_merge_avg'] = tc_merge[listColtc_merge].mean(axis=1)

s1 = extract_point(file_mean,date_range_cf,41,lat,lon,'sm',1)
listCols1 = []
for k in range(41):
   s1 = s1.rename(columns={k:'s1' + station_names[k] })
   s1['s1' + station_names[k]+'_ANOM'] = s1['s1' + station_names[k]]-np.mean(s1['s1' + station_names[k]])
   listCols1.append('s1' + station_names[k]+'_ANOM')
s1['s1_avg'] = s1[listCols1].mean(axis=1)

smap_l4 = extract_point(file_smapl4,date_range_cf,41,lat,lon,'sm',1)
listColsmap_l4 = []
for k in range(41):
   smap_l4 = smap_l4.rename(columns={k:'smap_l4' + station_names[k] })
   smap_l4['smap_l4' + station_names[k]+'_ANOM'] = smap_l4['smap_l4' + station_names[k]]-np.mean(smap_l4['smap_l4' + station_names[k]])
   listColsmap_l4.append('smap_l4' + station_names[k]+'_ANOM')
smap_l4['smap_l4_avg'] = smap_l4[listColsmap_l4].mean(axis=1)
...
for k in range(41):
   ...
   ascat12_5km['ascat12_5km' + station_names[k]+'_ANOM'] = ascat12_5km['ascat12_5km' + station_names[k]]-np.mean(ascat12_5km['ascat12_5km' + station_names[k]])
   listColascat12_5km.append('ascat12_5km' + station_names[k]+'_ANOM')
ascat12_5km['ascat12_5km_avg'] = ascat12_5km[listColascat12_5km].mean(axis=1)
...
listColcosmos = []
for k in range(41):
   station_data[station_names[k]+'_ANOM'] = station_data[station_names[k]]-np.mean(station_data[station_names[k]])
   listColcosmos.append(station_names[k]+'_ANOM')
station_data['_avg'] = station_data[listColcosmos].mean(axis=1)
...
ax1 = plt.subplot()
ax1.plot(new[site][mask].index,new[site][mask],'b.',label='COSMOS')
...
ax1.set_ylabel('SM anomalies \n' + r'$\,\mathrm{(m^3/m^3)}$',fontsize=11)
ax1.legend(loc='lower left', ncol=3,frameon=False,fontsize=11)
...
plt.savefig(plot_dir_1 + site +'_ANOM_time_series_mean_merged_with_avg2_'+version+'.png',dpi=300,bbox_inches='tight')
plt.close()
...
fig.autofmt_xdate()
#fig.text(0.145, 0.7, 'Site: '+site,fontsize=11)
plt.savefig(plot_dir_2 + site +'_ANOM_time_series_ascat_smap_'+version+'.png',dpi=300,bbox_inches='tight')
plt.close()
...
fig.autofmt_xdate()
#fig.text(0.145, 0.7, 'Site: '+site,fontsize=11)
plt.savefig(plot_dir_3 + site +'_ANOM_time_series_jules_merged_'+version+'.png',dpi=300,bbox_inches='tight')
plt.close()

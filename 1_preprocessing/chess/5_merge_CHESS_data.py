...
#-------------------------------------------------------------------------------
# Code to merge CHESS dataset into one large dataset
...
#####create target file
dataset = nc.Dataset(outFolder+'chess_all_uk.nc','w',format='NETCDF4')
# create dimensions
dataset.createDimension('lat',lat.shape[0])
dataset.createDimension('lon',lon.shape[0])
#dataset.createDimension('time',926)
...
for date_in in bla:
    ...
    file_chess =roodirc + 'chess-'+date_in.strftime('%Y') +'-'+ date_in.strftime('%m') +'-'+ date_in.strftime('%d')+'_reproject.nc'

    if os.path.isfile(file_chess) == True:
        sm_data =nc.Dataset(file_chess,'r')
        sm_chess = np.asarray(sm_data.variables['Band1'])
        sw_bsa[i,:] = sm_chess

    i = i+1
# close file
dataset.close()
os.chmod(file_chess,0664)

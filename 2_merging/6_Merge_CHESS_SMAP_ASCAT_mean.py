...
merge_method_TC=False
merge_method_Mean=True
...
if merge_method_Mean==True:
    dataset = nc.Dataset(merged_folder + 'merge_12.5km_chess_smap_ascat_mean.nc','w',format='NETCDF4')
...
############################# set the sample size threshold, it should be 100 as suggested by Gruber et al.,2016  ##########################################################
            if len(smos_tc[:,i,j].compressed())>100:
                ...
                smap_mean = np.nanmean(smap_merge[:,i,j])
                chess_mean = np.nanmean(chess_merge[:,i,j])
                smos_mean = np.nanmean(smos_merge[:,i,j])

                chess_scaled = b2*(chess_merge[:,i,j]-chess_mean)+smap_mean
                smos_scaled = b1*(smos_merge[:,i,j]-smos_mean)+smap_mean

                if  (weight >0.9) & (w1<1.1):
                    sm_cal[:,i,j] = np.sum([smap_merge[:,i,j]*w1*b1,chess_scaled*w2 ,smos_scaled*w3 ],0)
                else:
                    sm_cal[:,i,j] =  chess_scaled      ##np.nanmean()
...
# close file
dataset.close()
os.chmod(merged_folder + 'merge_12.5km_chess_smap_ascat_mean.nc',0664)

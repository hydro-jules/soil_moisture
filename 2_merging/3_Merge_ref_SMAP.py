...
b2_file = b2_folder + 'b2_interpolated_12.5km.nc'
# to derive mean smap value, I used this command:
# cdo -b F64 -timmean -setmissval,nan smap_12.5km.nc smap_12.5km_mean.nc
...
chess_scaled_interpolated_dataset = nc.Dataset(chess_folder + 'chess_scaled_interpolated_12.5km.nc','w',format='NETCDF4')
...
os.chmod(chess_folder + 'chess_scaled_interpolated_12.5km.nc',0664)
#*******************************************************************************************************
#*******************************************************************************************************

########create merged file###########################
if merge_method_TC==True:
    dataset = nc.Dataset(merged_folder + 'merge_12.5km_tc_ref_smap.nc','w',format='NETCDF4')
            ...
            if len(chess[:,i,j].compressed())>0:
                if len(chess_tc[:,i,j].compressed())>0:
                    ...
                    if  (weight >0.9) & (w1<1.1):
                        sm_cal[:,i,j] = np.sum([smap_merge[:,i,j]*w1,chess_scaled*w2 ,smos_scaled*w3 ],0)
                    else:
                        sm_cal[:,i,j] =  chess_full_scaled      ##np.nanmean()
                    ...
                    if  (weight >0.9) & (w1<1.1):
                        ...
                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
                        sm_cal[:,i,j] = np.where(np.isnan(sm_cal[:,i,j]),duo_1_sum,sm_cal[:,i,j])
##                    else:
##                        sm_cal[:,i,j] =  chess_full_scaled
                    ...
                    if  (weight >0.9) & (w1<1.1):
                        # fill in where NaN
                        duo_2_sum = np.sum([chess_duo2_scaled*w2_duo_2 ,smap_duo_2[:,i,j]*w1_duo_2],0)
                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
                        sm_cal[:,i,j] = np.where(np.isnan(sm_cal[:,i,j]),duo_2_sum,sm_cal[:,i,j])
##                    else:
##                        sm_cal[:,i,j] =  chess_full_scaled
        ...
        ##            elif (len(smos_tc[:,i,j].compressed())>0) & (len(smos_tc[:,i,j].compressed())<100):
         ...
         ##### To BE CONTINUED HERE>>>>
                    ...
                    if (p_value_ps>=0.05) & (p_value_pc>=0.05)& (p_value_sc>=0.05):
                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
                        sm_cal[:,i,j] = chess_full_scaled    #?????????? Not sure if correct....
                    elif (p_value_ps<0.05) & (p_value_pc<0.05)& (p_value_sc>=0.05):
                        sm_cal[:,i,j] = smap[:,i,j]
                    elif (p_value_ps<0.05) & (p_value_pc>=0.05)& (p_value_sc<0.05):
                        sm_cal[:,i,j] = smos_full_scaled
                    elif (p_value_ps>=0.05) & (p_value_pc<0.05)& (p_value_sc<0.05):
                        sm_cal[:,i,j] = chess_full_scaled
                    elif (p_value_ps>=0.05) & (p_value_pc<0.05)& (p_value_sc>=0.05):
                        sm_cal[:,i,j] = np.mean([chess_duo2_scaled,smap_duo_2[:,i,j]],0)
                    elif (p_value_ps<0.05) & (p_value_pc>=0.05)& (p_value_sc>=0.05):
                        sm_cal[:,i,j] = np.mean([smap_duo_3[:,i,j],smos_duo3_scaled],0)
                    elif (p_value_ps>=0.05) & (p_value_pc>=0.05)& (p_value_sc<0.05):
                        sm_cal[:,i,j] = np.mean([chess_duo1_scaled,smos_duo1_scaled],0)



##                    elif (p_value_ps<0.05) & (p_value_pc>=0.05)& (p_value_sc>=0.05):
##                     # calculate duo smos/smap
##                        w1_duo_3 = err_var[0]/np.sum([err_var[2],err_var[0]])  #smap
##                        w3_duo_3 = err_var[2]/np.sum([err_var[0],err_var[2]])  #smos
##
##                        duo_3_sum = np.sum([smap_duo_3[:,i,j]*w1_duo_3,smos_duo3_scaled*w3_duo_3 ],0)
##                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
##                        sm_cal[:,i,j] = duo_3_sum
##                    elif (p_value_ps>=0.05) & (p_value_pc<0.05)& (p_value_sc>=0.05):
##                        duo_2_sum = np.sum([chess_duo2_scaled*w2_duo_2 ,smap_duo_2[:,i,j]*w1_duo_2 ],0)
##                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
##                        sm_cal[:,i,j] = duo_2_sum
##                    elif (p_value_ps>=0.05) & (p_value_pc>=0.05)& (p_value_sc<0.05):
##                        duo_1_sum = np.sum([chess_duo1_scaled*w2_duo_1 ,smos_duo1_scaled*w3_duo_1 ],0)
##                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
##                        sm_cal[:,i,j] = duo_1_sum
##                    elif (p_value_ps<0.05) & (p_value_pc<0.05)& (p_value_sc<0.05):
##                        dummy = 0
##                    else:
##                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
##                        sm_cal[:,i,j] = chess_full_scaled
                ...
                sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
                sm_cal[:,i,j] = np.where(sm_cal[:,i,j]<0,chess_scaled_interpolated[:,i,j],sm_cal[:,i,j])
                sm_cal[:,i,j] = np.where(sm_cal[:,i,j]>1,chess_scaled_interpolated[:,i,j],sm_cal[:,i,j])
                sm_cal[:,i,j] = np.where(np.isnan(sm_cal[:,i,j]),chess_scaled_interpolated[:,i,j],sm_cal[:,i,j])
    ...
    ##                sm_cal[:,i,j] = chess_scaled
                ...
                #sm_cal[:,i,j] = np.where(np.isnan(sm_cal[:,i,j]),chess_full_scaled,sm_cal[:,i,j])
                sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
                sm_cal[:,i,j] = chess_scaled_interpolated[:,i,j]
...
# close file
dataset.close()
os.chmod(merged_folder + 'merge_12.5km_tc_ref_smap.nc',0664)

...
if merge_method_TC==True:
    dataset = nc.Dataset(merged_folder + 'weight2_12.5km.nc','w',format='NETCDF4')
...
sm_cal = dataset.createVariable('w2',np.float32,('lat','lon',),fill_value=-999.,zlib=True)

# create Attributes

sm_cal.units = 'unitless'
sm_cal.long_name = 'weight for CHESS (w2)'
...
# close file
dataset.close()
os.chmod(merged_folder + 'weight2_12.5km.nc',0664)

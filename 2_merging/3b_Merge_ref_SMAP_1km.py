...
if merge_method_TC==True:
    dataset = nc.Dataset(merged_folder + 'merge_1km_tc_ref_smap.nc','w',format='NETCDF4')
            ...
            ##            elif (len(smos_tc[:,i,j].compressed())>0) & (len(smos_tc[:,i,j].compressed())<100):
             ...
             ##### To BE CONTINUED HERE>>>>
                        ...
                        if (p_value_ps>=0.05) & (p_value_pc>=0.05)& (p_value_sc>=0.05):
                            ...
                            sm_cal[:,i,j] = np.mean([chess_duo2_scaled,smap_duo_2],0)
                        elif (p_value_ps<0.05) & (p_value_pc>=0.05)& (p_value_sc>=0.05):
                            sm_cal[:,i,j] = np.mean([smap_duo_3,smos_duo3_scaled],0)
                        elif (p_value_ps>=0.05) & (p_value_pc>=0.05)& (p_value_sc<0.05):
                            sm_cal[:,i,j] = np.mean([chess_duo1_scaled,smos_duo1_scaled],0)
                    except:

                        print smap_tc[:,i,j].compressed().shape
                        print smos_tc[:,i,j].compressed().shape
                        print chess_tc[:,i,j].compressed().shape
                        print 'Failed for Lat: '+  str(i+1) +' and Lon '+  str(j+1)
                        sm_cal[:,i,j] = np.ma.filled(sm_cal[:,i,j].astype(float), np.nan)
...
# close file
dataset.close()
os.chmod(merged_folder + 'merge_1km_tc_ref_smap.nc',0664)
